...
class GmailService {
  ...
  Future<List<EmailData>> searchSmsEmails({
    ...
    debugPrint('[GmailService] Searching for SMS emails with prefix: $subjectPrefix');
    onProgress?.call(const SyncProgress(status: 'Поиск SMS писем...'));

    // Build query for SMS emails
    String query = 'subject:"$subjectPrefix"';
    if (after != null) {
      final afterDate = '${after.year}/${after.month}/${after.day}';
      query += ' after:$afterDate';
    }
    debugPrint('[GmailService] SMS search query: $query');
    ...
    do {
      ...
      debugPrint('[GmailService] Found ${messages.length} SMS emails in this batch');

      onProgress?.call(SyncProgress(
        totalFound: totalFound,
        processed: totalProcessed,
        status: 'Найдено ~$totalFound SMS писем',
      ));

      // Fetch emails in parallel batches of 5
      final messagesToFetch = messages.where((_) => totalProcessed < maxResults).toList();
      const parallelBatchSize = 5;
      ...
      for (var i = 0; i < messagesToFetch.length; i += parallelBatchSize) {
        ...
        for (final emailData in results) {
          if (emailData != null) {
            allEmails.add(emailData);
            debugPrint('[GmailService] SMS email processed: ${emailData.subject}');
          }
          totalProcessed++;
        }
    ...
    debugPrint('[GmailService] SMS search complete. Total: ${allEmails.length}');
    onProgress?.call(SyncProgress(
      totalFound: allEmails.length,
      processed: allEmails.length,
      status: 'Завершено! Обработано ${allEmails.length} SMS',
    ));
